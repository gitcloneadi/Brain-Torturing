...
Given the 
root 
of a Binary Tree, your task is to return its 
...
A 
level order traversal
 is a breadth-first search (BFS) of the tree. It visits nodes level by level, starting from the root, and processes all nodes from left to right within each level before moving to the next.
...
Input:
 root = [10, 20, 30, N, N, 40, 50, N, N, N, N]
